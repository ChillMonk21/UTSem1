...
__global__ void find_matches_kernel(int* device_input, int length_match, int* binary_result)//, int* index_result)
{
	int index 				= (blockIdx.x * blockDim.x + threadIdx.x);
...
//  printf("Exclusive sum finished\n");	 
  ...
  //int *binary_check_array_new;      
  //cudaMalloc((void**)&binary_check_array_new, (length) * sizeof(int));

//  cudaMemcpy(binary_check_array,temp_out,(length)*sizeof(int),cudaMemcpyHostToDevice);

//testing purpose
  int length2 = nextPow2(length);
  exclusive_scan(binary_check_array, length2, binary_check_array); 	//TODO		
  cudaThreadSynchronize();

  cudaMemcpy(temp_out2,binary_check_array,(length)*sizeof(int),cudaMemcpyDeviceToHost);
  len_out_array[0] = temp_out2[length-1];
//  printf("length of input array = %d\n", length);
//  printf("length of output array = %d\n", len_out_array[0]);

//  for(int k = 0; k<length;k++)
//  {
//    printf("temp_out2[%d] = %d\n",k,temp_out2[k]);
//  }


//  printf("\n*****Final Stage to Compute Output Array*****\n");
  printf("blocks2 = %d, threadsperBlock2 = %d\n", blocks2, threadsperBlock2);
  find_matches_kernel<<<blocks2,threadsperBlock2>>>(binary_check_array,length, device_output);
  cudaThreadSynchronize();
...
*/
  ...
  //cudaFree(binary_check_array_new);    
