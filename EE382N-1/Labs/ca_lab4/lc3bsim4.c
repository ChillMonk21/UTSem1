...
int GetCHKUN(int *x)		{ return(x[CHKUN]);}
int GetCHKPROT(int *x)      { return(x[CHKPROT]);}
...
void eval_micro_sequencer() {
	...
	*/
	...
	int cond            	= GetCOND(CURRENT_LATCHES.MICROINSTRUCTION);
    	int excbit              = CURRENT_LATCHES.EXC; 
	...
	if(cond == 5){
    	...
    	else if((excbit == 1) && (cond = 6)){
		nextState	            = Low16bits(63);
        	NEXT_LATCHES.EXC        = 0x0;
	}
	else if(ird == 1){
...
void latch_datapath_values() {
   ...
   */  
	...
	int ldUSP 		= GetLD_USP(CURRENT_LATCHES.MICROINSTRUCTION); 
    	...
    	int checkUnalign	= GetCHKUN(CURRENT_LATCHES.MICROINSTRUCTION);
        int checkProt    	= GetCHKPROT(CURRENT_LATCHES.MICROINSTRUCTION);
	int ldPSR 		= GetLD_PSR(CURRENT_LATCHES.MICROINSTRUCTION);
    	...
    	if(ldMAR){
    		/*Protection Test*/
    		if(((Low16bits(BUS) >> 12) < 3) & (CURRENT_LATCHES.PSR15 == 1) & (checkProt == 1)){
    			NEXT_LATCHES.EXC 		= 0x1;
    			NEXT_LATCHES.EXCV 		= 0x2;
    		}
    		/*Ualigned Test*/
    		else if(((BUS & 0x1) ==1) & (checkUnalign == 1)){
        		NEXT_LATCHES.EXC 	        = 0x1;
    	...
    	if((CURRENT_LATCHES.STATE_NUMBER == 0xA) | (CURRENT_LATCHES.STATE_NUMBER == 0xB)){
    			NEXT_LATCHES.EXC 	= 0x1;
