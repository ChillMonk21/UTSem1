...
int marMuxOutput, pcOutput, shfOutput, aluOutput, mdrOutput; 
...
void eval_bus_drivers() {
   ...
   */    
    ...
    /*MARMUX*/ 
    if(marMux == 0){
        marMuxOutput        = Low16bits((CURRENT_LATCHES.IR << 1) & 0x1FF);
    }
    else{
        if(addr1Mux == 1){
        ...
        if(addr2Mux == 0){
            ...
            addr2MuxOutput      = Low16bits(pcOffset9 | ((pcOffset9 >> 9) ? 0xFE00 : 0));
        }
        else{
            pcOffset11          = Low16bits(CURRENT_LATCHES.IR & 0x7FF);
            addr2MuxOutput      = Low16bits(pcOffset11 | ((pcOffset11 >> 11) ? 0xF800 : 0));
        }
...
void drive_bus() {
   ...
   */    
    ...
    int gateMARMUX      = GetMARMUX(CURRENT_LATCHES.MICROINSTRUCTION);
   ...
   */  
    ...
    if(pcMux == 0){
        ...
        if(addr2Mux == 0){
            ...
            pcOffset9           = Low16bits(CURRENT_LATCHES.IR & 0x1FF);
            addr2MuxOutput      = Low16bits(pcOffset9 | ((pcOffset9 >> 9) ? 0xFFE000 : 0));
        }
        else{
            pcOffset11          = Low16bits(CURRENT_LATCHES.IR & 0x7FF);
            addr2MuxOutput      = Low16bits(pcOffset11 | ((pcOffset11 >> 11) ? 0xF800 : 0));
